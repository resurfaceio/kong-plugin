...
local function send_payload(self, conf, payload)
  ...
  headers_cache["Content-Encoding"] = "identity"
  headers_cache["User-Agent"] = "Resurface/1.1 (http_logger.py)"
...
function HttpLogHandler:access(conf)
  if pcall(kong.request.get_raw_body) then
    reqbod = kong.request.get_raw_body()
  end
...
function HttpLogHandler:log(conf)
  ...
  for k,v in pairs(serTable.request.headers) do
    table.insert(msg, {"request_header:" .. k, v})
  end
  for k,v in pairs(serTable.request.querystring) do
    table.insert(msg, {"request_param:" .. k, v})
  end
  if reqbod then
    table.insert(msg, {"request_body", reqbod})
  end
  reqbod = nil
  table.insert(msg, {"response_code", kong.service.response.get_status()})
  for k,v in pairs(kong.service.response.get_headers()) do
    table.insert(msg, {"response_header:" .. k, v})
  end
  if pcall(kong.service.response.get_raw_body) then
    table.insert(msg, {"response_body", kong.service.response.get_raw_body()})
  end
  --local entry = cjson.encode(kong.log.serialize())
  local entry = cjson.encode(msg)
